...
入力内容をチェックし、不備がある場合は登録できないようにしています。
...
Spring Bootを使用したREST APIを実装しています。
...
現在はデータベースを使用せず、Javaの `List<Place>` にデータを保持しています。
...
現在はSpring Bootを中心とした構成で動作しています。

```mermaid
flowchart LR
    A[Webブラウザ] -->|HTTP| B[Spring Boot]
    B --> C[Apache Tomcat]
    C --> D[PlaceController]
...
現在はデータベースを使用していないため、作業データはJavaの `List<Place>` に保持しています。
...
取得したプロジェクトをEclipseでMavenプロジェクトとしてインポートします。
...
- HTML・CSS・JavaScriptとSpring Bootを組み合わせたWebアプリケーションの構築
...
職業訓練でのWebアプリケーション開発課題として制作しました。

Java / Spring Bootを使用したWebアプリケーション開発を学習しながら、設計・実装・動作確認まで一連の開発工程を経験することを目的として制作しました。
